...
# ---- НАДЕВАЕМАЯ БРОНЯ (оверлеи поверх героя; те же якоря головы/торса) ----
# пропорции: голова y~0.22, плечи y~0.36 (шире), талия y~0.62
def wa_helm(g, p=None):
    n = g.n; cx = n/2; st = PAL['steel']; gold = PAL['gold']
    hy = n*0.25; hr = n*0.25                                    # голова 7 строк — чуть выше
    g.ellipse(cx, hy-n*0.05, hr, hr*0.70, st[0])                 # шапка-шлем по верху головы
    g.rect(cx-hr, hy-n*0.01, cx+hr, hy+n*0.03, st[1])          # кромка
    g.rect(cx-1.4, hy-hr-n*0.02, cx+1.4, hy-n*0.05, gold[0])   # гребешок
    g.ellipse(cx, hy-hr-n*0.03, n*0.03, n*0.03, gold[0])        # шарик на макушке

def wa_chest(g, p=None):
    n = g.n; cx = n/2; st = PAL['steel']; gold = PAL['gold']
    g.ellipse(cx, n*0.61, n*0.13, n*0.10, st[0])               # компактный нагрудник
    g.ellipse(cx-n*0.05, n*0.58, n*0.08, n*0.05, st[2])        # верхний блик
    g.line(cx, n*0.55, cx, n*0.67, st[1], 1)                    # шов
    g.ellipse(cx, n*0.60, n*0.025, n*0.02, gold[0])           # эмблема

def wa_shoulders(g, p=None):
    n = g.n; cx = n/2; st = PAL['steel']
    for s in (-1, 1):
        g.ellipse(cx+s*n*0.14, n*0.54, n*0.055, n*0.045, st[0])  # мини-наплечники
        g.ellipse(cx+s*n*0.14, n*0.52, n*0.045, n*0.03, st[2])
...
RACE_CFG = {
    'human':     {'body': ((58,110,196),(30,60,124),(150,196,255)), 'skin': ((232,196,158),(184,144,110)), 'feat': 'hood', 'style': 'std', 'hair': (110,74,44), 'trim': (240,208,90), 'hat': (150,40,44)},
    'beastkin':  {'body': ((150,108,66),(94,64,40),(206,164,120)), 'skin': ((222,190,150),(172,140,106)), 'feat': 'ears', 'style': 'std', 'hair': (120,84,48), 'trim': (240,224,150), 'hat': (200,80,70)},
    'dwarf':     {'body': ((166,66,54),(100,34,30),(220,112,92)), 'skin': ((230,186,146),(182,142,108)), 'feat': 'beard', 'style': 'stocky', 'beard': (216,140,60), 'trim': (222,190,90), 'hat': (176,184,198)},
    'undead':    {'body': ((78,94,88),(42,54,50),(140,156,146)), 'skin': ((196,210,196),(140,158,142)), 'feat': 'hood', 'style': 'robe', 'hair': (60,74,64), 'eyeglow': (150,245,168), 'trim': (110,200,150), 'hat': (40,54,48)},
    'demon':     {'body': ((160,46,42),(90,20,22),(224,94,74)), 'skin': ((204,90,76),(142,52,48)), 'feat': 'horns', 'style': 'spiky', 'horn': (48,24,26), 'eyeglow': (255,200,76), 'trim': (255,150,60), 'hat': (34,24,28)},
    'elf':       {'body': ((56,150,118),(32,98,78),(150,218,184)), 'skin': ((232,212,176),(186,164,132)), 'feat': 'longears', 'style': 'robe', 'hair': (222,208,158), 'trim': (240,214,120), 'hat': (60,150,118)},
    'golem':     {'body': ((118,124,140),(66,72,86),(200,206,220)), 'skin': ((142,146,162),(90,94,112)), 'feat': 'stone', 'style': 'rock', 'eyeglow': (150,226,255), 'trim': (150,226,255), 'hat': (100,104,120)},
    'dragonkin': {'body': ((70,148,86),(38,98,50),(150,208,118)), 'skin': ((110,172,96),(70,120,64)), 'feat': 'scales', 'style': 'std', 'horn': (56,94,52), 'eyeglow': (245,224,92), 'trim': (240,220,120), 'hat': (56,94,52), 'wing': (44,120,70)},
}

# Герой в стиле чиби: БОЛЬШАЯ голова (по размеру ≈ тело), компактное узкое
# тело без шеи, короткие малозаметные ножки. Больше деталей в лице/уборе.
# Голова/торс на якорях — совпадают с бронёй-оверлеями (см. wa_* фракции).
W = 14  # низкое разрешение — крупные пиксели

def _row(*segs):
    r = ''.join(ch * c for ch, c in segs)
    return (r[:W]).ljust(W, '.')

# Голова 3/4 сбоку (лицо повёрнуто вправо). Токены: h=убор/волосы, H=тень убора,
# r=прядь волос, s=кожа, d=тень кожи, o=контур, e=глаз, w=блик, g=свеч/рог.
# 7 строк (голова чуть меньше — взрослее пропорции), с насуплённой бровью.
def _head(kind):
    base = [
        _row(('.',4),('o',6),('.',4)),                                              # 0 макушка
        _row(('.',3),('o',1),('h',2),('r',3),('h',1),('o',1),('.',3)),              # 1 волосы+прядь
        _row(('.',3),('o',1),('h',2),('s',4),('o',1),('.',3)),                      # 2 лоб
        _row(('.',3),('o',1),('s',2),('o',3),('s',1),('o',1),('.',3)),              # 3 насуплённая бровь
        _row(('.',3),('o',1),('s',2),('e',1),('w',1),('s',1),('o',1),('.',4)),      # 4 узкий глаз+блик
        _row(('.',3),('o',1),('s',5),('d',1),('o',1),('.',3)),                      # 5 скула (тень)
        _row(('.',4),('o',1),('s',4),('o',1),('.',4)),                              # 6 квадратная челюсть
    ]
    if kind == 'hood':
        base[1] = _row(('.',3),('o',1),('h',6),('o',1),('.',3))
        base[2] = _row(('.',3),('o',1),('h',1),('s',4),('h',1),('o',1),('.',3))
        base[6] = _row(('.',3),('o',1),('h',1),('s',4),('o',1),('.',4))
    elif kind == 'ears':
        base[0] = _row(('.',3),('o',1),('h',1),('o',1),('.',2),('o',1),('h',1),('o',1),('.',3))
        base[1] = _row(('.',3),('o',1),('r',6),('o',1),('.',3))
    elif kind == 'helm':
        base[0] = _row(('.',4),('o',1),('g',1),('o',2),('g',1),('o',1),('.',4))
        base[1] = _row(('o',1),('H',1),('o',1),('h',8),('o',1),('H',1),('o',1))
        base[2] = _row(('.',3),('o',1),('h',6),('o',1),('.',3))
        base[3] = _row(('.',3),('o',1),('h',1),('o',1),('s',1),('e',1),('w',1),('s',1),('o',1),('.',3))  # забрало+глаз
    elif kind == 'horns':
        base[0] = _row(('g',1),('o',1),('.',3),('o',4),('.',3),('o',1),('g',1))
        base[1] = _row(('.',1),('g',1),('o',1),('h',6),('o',1),('g',1),('.',1))
    elif kind == 'wizard':
        base[0] = _row(('.',6),('o',2),('.',6))
        base[1] = _row(('.',5),('o',1),('h',2),('o',1),('.',5))
        base[2] = _row(('.',4),('o',1),('h',4),('o',1),('.',4))
        base[3] = _row(('.',2),('o',1),('t',3),('r',2),('t',3),('o',1),('.',2))     # брим шляпы
    elif kind == 'stone':
        base[1] = _row(('.',3),('o',1),('h',6),('o',1),('.',3))
        base[3] = _row(('.',3),('o',1),('h',6),('o',1),('.',3))                     # каменный навес-бровь
        base[4] = _row(('.',3),('o',1),('s',2),('g',2),('s',1),('o',1),('.',4))     # светящийся глаз
        base[5] = _row(('.',3),('o',1),('s',1),('C',1),('s',4),('o',1),('.',3))     # трещина
    elif kind == 'dragon':
        base[0] = _row(('g',1),('o',1),('.',3),('o',4),('.',3),('o',1),('g',1))
        base[1] = _row(('.',2),('o',1),('r',8),('o',1),('.',2))
        base[5] = _row(('.',3),('o',1),('s',6),('o',1),('s',1),('.',2))             # морда-выступ
    return base

def _put(rows, ry, cx, ch):
    if 0 <= ry < len(rows) and 0 <= cx < W:
        s = rows[ry]
        rows[ry] = (s[:cx] + ch + s[cx+1:])[:W].ljust(W, '.')

# Расовые «фишки»-придатки поверх силуэта: крылья/хвост/уши — по краям, за корпусом.
# Индексы: голова 0..7, корпус 8..11, ноги 12..13.
def _features(rows, feat, frame):
    if feat == 'scales':      # драконид — перепончатые крылья за спиной
        for ry, cx, ch in [
            (7,2,'o'),(7,11,'o'),
            (8,1,'o'),(8,2,'v'),(8,11,'v'),(8,12,'o'),
            (9,0,'o'),(9,1,'v'),(9,2,'v'),(9,11,'v'),(9,12,'v'),(9,13,'o'),
            (10,1,'o'),(10,2,'v'),(10,11,'v'),(10,12,'o')]:
            _put(rows, ry, cx, ch)
    elif feat == 'horns':     # демон — хвост со стрелкой
        for ry, cx, ch in [(11,11,'k'),(12,12,'k'),(13,12,'k'),(13,13,'g')]:
            _put(rows, ry, cx, ch)
    elif feat == 'ears':      # бистккин — пушистый хвост
        for ry, cx, ch in [(11,11,'r'),(12,11,'r'),(12,12,'r'),(13,12,'r')]:
            _put(rows, ry, cx, ch)
    elif feat == 'longears':  # эльф — длинное острое ухо назад
        for ry, cx, ch in [(3,2,'s'),(4,1,'o'),(4,2,'s')]:
            _put(rows, ry, cx, ch)

# Силуэт корпуса+ног (ровно 6 рядов) — свой у каждого стиля, чтобы формы не
# были одинаковыми. Затенение: l=блик слева, C=тень справа, g=эмблема/пряжка.
def _body(style, frame):
    dx = 0 if frame == 0 else 1  # микро-сдвиг для шага
    # ноги из 3 сегментов (бедро/голень/сапог) — взрослая пропорция, не куцые.
    def legs3(thigh='c', shin='C', foot='b', w=2, pad=4):
        gap = 14 - 2 * pad - 2 * w
        return [
            _row(('.',pad),(thigh,w),('.',gap),(thigh,w),('.',pad)),
            _row(('.',pad+dx),(shin,w),('.',gap),(shin,w),('.',pad-dx)),
            _row(('.',pad+dx),(foot,w),('.',gap),(foot,w),('.',pad-dx)),
        ]
    if style == 'robe':          # маг/жрец: длинная мантия с расширяющимся подолом
        hem = 3 - (frame & 1)
        return [
            _row(('.',4),('o',1),('l',1),('c',2),('C',1),('o',1),('s',1),('.',3)),          # плечи+кисть
            _row(('.',4),('o',1),('c',1),('t',2),('c',1),('o',1),('.',4)),                  # застёжка мантии
            _row(('.',3),('o',1),('l',1),('c',2),('C',1),('c',1),('o',1),('.',3)),          # мантия шире
            _row(('.',3),('o',1),('c',1),('C',1),('c',2),('C',1),('o',1),('.',3)),          # складки
            _row(('.',3),('o',1),('c',1),('C',1),('c',2),('C',1),('o',1),('.',3)),          # складки ниже
            _row(('.',hem),('o',1),('t',8-2*(hem-2)),('o',1),('.',hem)),                    # кант подола (колышется)
            _row(('.',hem),('o',1),('C',8-2*(hem-2)),('o',1),('.',hem)),                    # тень под подолом
        ]
    if style == 'stocky':        # дворф: широкий приземистый, толстые сапоги
        return [
            _row(('.',3),('o',1),('t',6),('o',1),('.',3)),                                  # широкий воротник
            _row(('.',3),('o',1),('l',1),('c',4),('C',1),('o',1),('s',1),('.',2)),          # широкие плечи+кисть
            _row(('.',3),('o',1),('c',1),('g',1),('C',1),('c',2),('o',1),('.',3)),          # грудь+пряжка
            _row(('.',3),('o',1),('t',6),('o',1),('.',3)),                                  # массивный пояс
            *legs3('c', 'b', 'o', w=3, pad=3),                                              # толстые короткие ноги
        ][:7]
    if style == 'rock':          # голем: каменные плиты, трещины, светящееся ядро
        return [
            _row(('.',3),('o',1),('h',6),('o',1),('.',3)),                                  # каменные плечи
            _row(('.',3),('o',1),('c',1),('C',1),('l',1),('c',2),('o',1),('s',1),('.',2)),  # плита+кисть
            _row(('.',3),('o',1),('c',2),('g',1),('C',2),('o',1),('.',3)),                  # светящееся ядро
            _row(('.',3),('o',1),('C',1),('c',2),('C',1),('c',1),('o',1),('.',3)),          # трещина
            *legs3('c', 'C', 'o', w=3, pad=3),                                              # массивные каменные ноги
        ][:7]
    if style == 'spiky':         # демон: шипастые наплечники
        return [
            _row(('.',4),('o',1),('t',4),('o',1),('.',4)),                                  # воротник
            _row(('.',2),('o',1),('.',1),('o',1),('c',2),('o',1),('.',1),('o',1),('s',1),('.',1)),  # шипы по плечам+кисть
            _row(('.',4),('o',1),('c',1),('g',1),('C',1),('c',1),('o',1),('.',4)),          # грудь+эмблема
            _row(('.',4),('o',1),('t',1),('g',1),('t',2),('o',1),('.',4)),                  # пояс
            *legs3('c', 'C', 'b', w=2, pad=4),                                              # ноги
        ][:7]
    # std — базовый воин: плащ-кант, наплечники-блик, эмблема, пряжка
    return [
        _row(('.',4),('o',1),('t',4),('o',1),('.',4)),                                      # воротник
        _row(('.',4),('o',1),('l',1),('c',2),('C',1),('o',1),('s',1),('.',3)),              # плечи+блик+кисть
        _row(('.',4),('o',1),('c',1),('g',1),('C',1),('c',1),('o',1),('.',4)),              # грудь+эмблема
        _row(('.',4),('o',1),('t',1),('g',1),('t',2),('o',1),('.',4)),                      # пояс+пряжка
        *legs3('c', 'C', 'b', w=2, pad=4),                                                  # ноги (бедро/голень/сапог)
    ][:7]
...
def f_race(g, cfg, frame):
    ...
    hat = cfg.get('hat', bsh); hsh = dark(hat, 0.72)
    hair = cfg.get('hair', hat)
    gl = cfg.get('eyeglow')
    boot = dark(bsh, 0.62)
    PAL = {
        'o': (20, 16, 24), 's': skin, 'd': sksh, 'c': body, 'C': bsh, 'l': blt,
        't': trim, 'h': hat, 'H': hsh, 'r': hair, 'b': boot,
        'e': (gl if gl else (34, 28, 42)), 'w': (250, 250, 255),
        'g': (gl if gl else (230, 210, 90)),
        'v': cfg.get('wing', blt), 'k': bsh,
    }
    feat = cfg['feat']
    kind = {'hood': 'hood', 'ears': 'ears', 'beard': 'helm', 'horns': 'horns',
            'longears': 'wizard', 'stone': 'stone', 'scales': 'dragon'}.get(feat, 'hood')
    rows = _head(kind)[:]
    # КОМПАКТНОЕ узкое тело без шеи (ЯВНО уже головы). Силуэт корпуса — свой на
    # каждый стиль расы (мантия/приземистый/камень/шипы/база), чтобы формы различались.
    rows += _body(cfg.get('style', 'std'), frame)
    # окладистая борода дворфа — в нижней части головы (7-строчная голова: ряды 5–6)
    if feat == 'beard':
        rows[5] = _row(('.',3),('o',1),('g',6),('o',1),('.',3))
        rows[6] = _row(('.',4),('o',1),('g',4),('o',1),('.',4))
    # расовые придатки (крылья/хвост/уши)
    _features(rows, feat, frame)
    H = len(rows)
    bs = 3
    ox = (g.n - W * bs) // 2; oy = (g.n - H * bs) // 2
    for ry, line in enumerate(rows):
        for rx, chp in enumerate(line):
            col = PAL.get(chp)
            if col is not None:
                for dy in range(bs):
                    for dx in range(bs):
                        g.set(ox + rx * bs + dx, oy + ry * bs + dy, col)
...
def build_creature(name, fn, pal, kw, n, scale, mirror=True, raw=False):
    random.seed(name)
    g = Grid(n)
    fn(g, pal, **kw)
    if mirror:
        g.mirror_lr()
    if not raw:            # raw=True для пиксель-арта (свой контур, без сглаживания)
        g.shade()
        g.outline()
    ...
    for race, cfg in RACE_CFG.items():
        for fr in (0, 1):
            build_creature('hero_%s_%d' % (race, fr),
                           ...
                           None, {}, 48, 4, mirror=False, raw=True)
    # запасной 'hero' = человек, кадр 0
    build_creature('hero', (lambda g, p: f_race(g, RACE_CFG['human'], 0)), None, {}, 48, 4, mirror=False, raw=True)
    build_aura()
